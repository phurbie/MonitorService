...
namespace MonitorService
{
    public class SNMPTrap : ServiceBase
    {
        ...
        private void InitializeComponent()
        {
            ...
            _webServer = new WebServer(_sqlStorage, new SSLCertificate(), new RemoteCalls());
        }
        ...
        private void ProcessSnmpPacket(byte[] data, IPEndPoint remoteEndPoint)
        {
            try
            {
                ...
                if (data[offset++] != 0x30)
                    errorInfo += $"Expected 0x30 at offset 0, got {data[0]:X2}; ";

                int totalLength = DecodeBerLength(data, ref offset);
                if (offset + totalLength > data.Length)
                    errorInfo += $"Invalid total length: {totalLength} at offset {offset}; ";

                string snmpVersion = "";
                if (data[offset++] != 0x02)
                    errorInfo += $"Missing version marker 0x02, got {data[offset - 1]:X2}; ";

                int versionLength = DecodeBerLength(data, ref offset);
                int version = 0;
                for (int i = 0; i < versionLength; i++)
                    version = (version << 8) | data[offset++];
                snmpVersion = $"SNMPv{version + 1}";

                string community = "";
                if (data[offset++] != 0x04)
                    errorInfo += $"Missing community marker 0x04, got {data[offset - 1]:X2}; ";
                ...
                if (pduTag != 0xA7 && pduTag != 0xA4)
                    errorInfo += $"Unsupported PDU tag 0x{pduTag:X2}";
                offset++;
                ...
                {
                    if (data[offset++] != 0x02)
                        errorInfo += $"Expected request-id INTEGER";
                    int len = DecodeBerLength(data, ref offset);
                    requestId = 0;
                    for (int i = 0; i < len; i++)
                        requestId = (requestId << 8) | data[offset++];
                    request += ($"RequestID: {requestId} ");

                    if (data[offset++] != 0x02)
                        errorInfo += "Expected error-status INTEGER";
                    len = DecodeBerLength(data, ref offset);
                    errorStatus = 0;
                    for (int i = 0; i < len; i++)
                        errorStatus = (errorStatus << 8) | data[offset++];

                    if (data[offset++] != 0x02)
                        throw new Exception("Expected error-index INTEGER");
                    len = DecodeBerLength(data, ref offset);
                    errorIndex = 0;
                    for (int i = 0; i < len; i++)
                        errorIndex = (errorIndex << 8) | data[offset++];

                    if (errorStatus != 0 || errorIndex != 0)
                        errorInfo += $"Error: {errorStatus}/{errorIndex} ";
                }
                else if (pduTag == 0xA4)
                {
                    if (data[offset++] != 0x06)
                        errorInfo += "Missing enterprise OID";
                    ...
                    if (data[offset++] != 0x40)
                        errorInfo += "Missing agent address";
                    ...
                    if (data[offset++] != 0x02)
                        errorInfo += "Missing generic trap";
                    int genLen = DecodeBerLength(data, ref offset);
                    int genericTrap = 0;
                    for (int i = 0; i < genLen; i++)
                        genericTrap = (genericTrap << 8) | data[offset++];
                    request += $"Generic: {genericTrap} ";

                    if (data[offset++] != 0x02)
                        errorInfo += ("Missing specific trap");
                    int specLen = DecodeBerLength(data, ref offset);
                    int specificTrap = 0;
                    for (int i = 0; i < specLen; i++)
                        specificTrap = (specificTrap << 8) | data[offset++];
                    request += $"Specific: {specificTrap} ";

                    if (data[offset++] != 0x43)
                        errorInfo += ("Missing timestamp");
                ...
                _sqlStorage.StoreSNMPData(DateTime.Now, remoteEndPoint.Address.ToString(),
                    remoteEndPoint.Port, errorInfo, snmpVersion, community, pdu, request, varBind, hexData);
            }
            catch (Exception ex)
            {
                ...
                string hexData = BitConverter.ToString(data).Replace("-", " ");
                _sqlStorage.StoreSNMPData(DateTime.Now, remoteEndPoint.Address.ToString(),
                    remoteEndPoint.Port, $"Parsing failed: {ex.Message}", "", "", "", "", "", hexData);
            }
        }
